...
<div class="analyses-orders-view">
    ...
    <p>
        <?php if ($sum > 0): ?>
            ...
            <?= Html::a('<i class="bi bi-arrow-down-circle"></i>&nbsp;' . Yii::t('lang', 'Expand Invoice'), '#collapse-print-invoice-content', ['class' => 'btn btn-warning', 'data-toggle' => 'collapse', 'role' => 'button', 'aria-expanded' => 'true']) ?>
    ...
    <?php if ($sum > 0): ?>
        <div class="collapse <?= $model->status != $model::STATUS_DONE ? 'show' : '' ?>" id="collapse-print-invoice-content">
                    ...
                    <thead>
                        ...
                        <?php $i = 1; $old_cat_title = ''; foreach ($analyses as $analys): ?>
                            ...
                            <tr>
                                <td><b><?= $i ?></b></td>
                                <td><?= $analys['title'] ?></td>
                                <td style="font-size: 25px;"><b><?= isset(json_decode($model->analyses_values, true)[$analys['id']]) ? json_decode($model->analyses_values, true)[$analys['id']] : '' ?></b></td>
                                <td><?= $analys['units'] ?></td>
