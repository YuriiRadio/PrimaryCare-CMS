...
<div class="analyses-orders-form">
    ...
    <div class="row align-items-center">
        ...
        <div class="col-3">
            <?php echo $form->field($model, 'doctor_id')->dropDownList($doctors) ?>
        </div>
        <div class="col-5">
            <?php
            $url = \yii\helpers\Url::to(['/admin/patients/patients-list']);
            $dataList = Patients::find()->where(['id' => $model->patient_id])->asArray()->all();
            ...
            echo $form->field($model, 'patient_id')->widget(Select2::classname(), [
                ...
                'pluginOptions' => [
                    ...
                    'templateSelection' => new JsExpression("function(patients) "
                            . "{ console.log(patients); $('#analysesorders-patient_id option[our_patient]:selected').each(function(){ if (Number($(this).attr('our_patient'))) { $('#our_patient').prop('checked', true) ;} });"
                            . "if (typeof(patients.our_patient)!='undefined' && Number(patients.our_patient)) { $('#our_patient').prop('checked', true); "
                            . "} else if (typeof(patients.our_patient)!='undefined' && !Number(patients.our_patient) && $('#our_patient').prop('checked')) { "
                            . "$('#our_patient').prop('checked', false); } "
        ...
        <div class="col-2">
            <?php
                echo '<div class="form-group required">';
                    ...
                    echo Html::input('checkbox', null, null, ['id' => 'our_patient', 'class' => 'form-control', 'disabled' => false]);
                echo '</div>';
            ?>
        </div>
    </div>
                    ...
                    <table class="table table-hover" <?= $model->isNewRecord ? 'style="background: #FF9200"' : 'style="background: #00cc66"' ?>>
                        ...
                        <?php $i = 1; foreach ($analyses_packages as $analys_package): ?>
                            <tr>
                                ...
                                <td><?= Html::input('checkbox', null, null, ['class' => 'form-control', 'data' => ['num' => $analys_package['pac_num'], 'is_free' => $analys_package['is_free']]]) ?></td>
